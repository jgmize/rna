...
class RestModelClientTest(TestCase):
    ...
    def test_restore(self, mock_hypermodel):
        ...
        fields = [mock_fk_field, 'non_fk_field']
        mock_restore_object = Mock(return_value='the dude abides')
        mock_save_object = Mock()
        mock_serializer = Mock(
            Meta=Mock(model=Mock(_meta=Mock(fields=fields))),
            restore_object=mock_restore_object, save_object=mock_save_object)
        data = {'url': 'http://remove.me', 'fk': 'http://thedu.de'}
        mock_hypermodel.return_value = 'abides'

        rc = clients.RestModelClient()
        instance = rc.restore(mock_serializer, data)
        eq_(instance, 'the dude abides')
        mock_hypermodel.assert_called_once_with('http://thedu.de', 'to', False)
        mock_restore_object.assert_called_once_with({'fk': 'abides'})
        eq_(mock_save_object.called, 0)
    ...
    def test_restore_save_modified(self, mock_hypermodel):
        ...
        mock_save_object = Mock()
        mock_serializer = Mock(
            Meta=Mock(model=Mock(_meta=Mock(fields=[]))),
            restore_object=mock_restore_object, save_object=mock_save_object)
        ...
        mock_save_object.assert_called_once_with(instance, modified=True)
    ...
    def test_post_instance(self, mock_post, mock_serialize):
        ...
        mock_post.assert_called_once_with(
            'http://positronic.net', 'serialized')
    ...
    def test_put_instance(self, mock_put, mock_serialize):
        ...
        eq_(response, 'response')
        mock_put.assert_called_once_with('http://positronic.net', 'serialized')
        mock_serialize.assert_called_once_with(instance)
    ...
    def test_hypermodel_exists(self):
        mock_get = Mock(return_value='what was the question?')
        mock_model_class = Mock(objects=Mock(get=mock_get))
        rc = clients.RestModelClient()
    ...
    def test_hypermodel_does_not_exist(self, mock_model_client):
        mock_get = Mock(side_effect=ObjectDoesNotExist)
        mock_model_class = Mock(objects=Mock(get=mock_get))
...
class RNASyncCommandTest(TestCase):
    ...
    def test_model_params_legacy_api(self):
        ...
        params = rnasync.Command().model_params(models, legacy_api)
